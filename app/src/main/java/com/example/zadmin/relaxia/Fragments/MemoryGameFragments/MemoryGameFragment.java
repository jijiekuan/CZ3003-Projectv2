...
public class MemoryGameFragment extends Fragment implements EventForMemoryGameFragment {
    ...
    public void onEvent(Event event) {
        if(event instanceof FlipDownCardsEvent){
            onEvent(new FlipDownCardsEvent());
        }
        else if(event instanceof HidePairCardsEvent){
            int id1 = ((HidePairCardsEvent) event).id1;
            int id2 = ((HidePairCardsEvent) event).id2;
            onEvent(new HidePairCardsEvent(id1,id2));
        }
        else if(event instanceof GameWonEvent){
            GameState gamestate = ((GameWonEvent)event).gameState;
            onEvent(new GameWonEvent(gamestate));
        }
    }
